...
from pymodbus.exceptions import NotImplementedException, NoSuchSubordinateException
...
class ModbusBaseRequestHandler(asyncio.BaseProtocol):
    """ Implements modbus subordinate wire protocol
    ...
    async def handle(self):
        ...
        the modbus subordinate and main
        ...
        while self.running:
            try:
                units = self.server.context.subordinates()
    ...
    def execute(self, request, *addr):
        ...
        try:
            if self.server.broadcast_enable and request.unit_id == 0:
                broadcast = True
                # if broadcasting then execute on all subordinate contexts, note response will be ignored
                for unit_id in self.server.context.subordinates():
                    response = request.execute(self.server.context[unit_id])
            else:
                context = self.server.context[request.unit_id]
                response = request.execute(context)
        except NoSuchSubordinateException as ex:
            _logger.debug("requested subordinate does "
                          "not exist: %s" % request.unit_id )
            if self.server.ignore_missing_subordinates:
...
class ModbusTcpServer:
    ...
    def __init__(self,
        ...
        :param backlog:  is the maximum number of queued connections
                    ...
                    connections are being made and broken to your Modbus subordinate
