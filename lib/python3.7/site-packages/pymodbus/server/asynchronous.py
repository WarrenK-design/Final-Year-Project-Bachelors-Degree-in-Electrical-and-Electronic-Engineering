...
from pymodbus.exceptions import NoSuchSubordinateException
...
class ModbusTcpProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        ...
        if not self.factory.control.ListenOnly:
            units = self.factory.store.subordinates()
    ...
    def _execute(self, request):
        ...
        except NoSuchSubordinateException as ex:
            _logger.debug("requested subordinate does not exist: %s" % request.unit_id )
            if self.factory.ignore_missing_subordinates:
                return # the client will simply timeout waiting for a response
            response = request.doException(merror.GatewayNoResponse)
        except Exception as ex:
            _logger.debug("Datastore unable to fulfill request: %s" % ex)
            response = request.doException(merror.SubordinateFailure)
...
class ModbusServerFactory(ServerFactory):
    ...
    def __init__(self, store, framer=None, identity=None, **kwargs):
        ...
        :param ignore_missing_subordinates: True to not send errors on a request to a missing subordinate
        """
        ...
        self.ignore_missing_subordinates = kwargs.get('ignore_missing_subordinates', Defaults.IgnoreMissingSubordinates)
...
class ModbusUdpProtocol(protocol.DatagramProtocol):
    ...
    def __init__(self, store, framer=None, identity=None, **kwargs):
        ...
        :param framer: The framer strategy to use
        :param identity: An optional identify structure
        :param ignore_missing_subordinates: True to not send errors on a request to
        a missing subordinate
        """
        ...
        self.control = ModbusControlBlock()
        self.access = ModbusAccessControl()
        self.ignore_missing_subordinates = kwargs.get('ignore_missing_subordinates',
                                                Defaults.IgnoreMissingSubordinates)
    ...
    def _execute(self, request, addr):
        ...
        try:
            ...
            _logger.debug("requested subordinate does not exist: "
                          "%s" % request.unit_id )
            if self.ignore_missing_subordinates:
                return # the client will simply timeout waiting for a response
            response = request.doException(merror.GatewayNoResponse)
        except Exception as ex:
            _logger.debug("Datastore unable to fulfill request: %s" % ex)
...
def StartTcpServer(context, identity=None, address=None,
    ...
    :param ignore_missing_subordinates: True to not send errors on a request \
    to a missing subordinate
...
def StartSerialServer(context, identity=None, framer=ModbusAsciiFramer,
    ...
    :param console: A flag indicating if you want the debug console
    :param ignore_missing_subordinates: True to not send errors on a request to a
           missing subordinate
    :param defer_reactor_run: True/False defer running reactor.run() as part
           of starting server, to be explictly started by the user
    :param custom_functions: An optional list of custom function classes
